d3.computeIntersections = function(maps,originalMapping)
{
  //http://stackoverflow.com/questions/26028124/the-intersection-of-all-combinations-of-n-sets
    ...
    function mergeSet(setA,setB) {
        var map = {};
        for (var i=0;(i<setA.elements.length || i<setB.elements.length);i++) {
            if (i<setA.elements.length) map[setA.elements[i]]=true;
            if (i<setB.elements.length) map[setB.elements[i]]=true;
        }
        setA.elements = Object.keys(map);
    }
